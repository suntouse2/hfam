...
import z from "zod";
export async function viewMethod(connectorId) {
    const method = await methodsApi.getMethod(connectorId);
    const kb = new InlineKeyboard();
    const lines = [];
    const provider = method.byProvider ? await providersApi.getProvider(method.byProvider) : null;
    ...
    kb.text(`${method.showLabel ? '✅ Имя отображается' : '❌ Имя не отображается'}`, `method:configure:showLabel:${method.showLabel ? 'false' : 'true'}`);
    kb.row();
    if (method.imageSrc && z.string().url().safeParse(method.imageSrc).success) {
        kb.url('🏞️ Иконка', method.imageSrc).row();
    }
    kb.text(`🌐 Провайдер: ${method?.connector?.name || 'Не выбран'}`, `method:configure:connector`);
    kb.row();
    if (!method.connector) {
