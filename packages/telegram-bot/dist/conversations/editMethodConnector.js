...
export default async function editMethodConnector(conversation, ctx) {
    ...
    const connectors = await conversation.external(()=>connectorsApi.getConnectors({
    ...
    if (data === 'pc:delete') {
        ...
        await conversation.external(()=>methodsApi.updateMethod(methodId, {
                connectorId: Number(data.split(':')[1])
            }));
    }
