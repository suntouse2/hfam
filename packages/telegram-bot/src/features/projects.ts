...
import editProjectSupportLink from "@/conversations/editProjectSupportLink";
...
projects.use(createConversation(editProjectSupportLink));
...
projects.callbackQuery("project:configure:supportLink", async (ctx) => {
	await ctx.answerCallbackQuery();
	await ctx.conversation.enter("editProjectSupportLink");
});
