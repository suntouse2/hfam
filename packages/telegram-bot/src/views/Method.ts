...
export async function viewMethod(connectorId: number) {
	...
	lines.push(
		`🌐 Название: <b>${method.label || "Без имени"}</b> (ID: ${method.id})`,
	);
	lines.push(" ");
	...
	if (!method.connector) {
		kb.text(
			`🔗 Шлюз: ${provider?.title || "Не выбран"}`,
			`method:configure:provider`,
		);
		kb.row();
		kb.text(
			`🧩 Метод: ${method.method || "Не выбран"}`,
			`method:configure:methods`,
		);
		kb.row();
	}
	kb.text(`Позиция: ${method.position}`, `method:configure:position`);
	kb.row();
