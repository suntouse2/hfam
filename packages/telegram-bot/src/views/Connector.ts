...
export async function viewConnector(connectorId: number) {
	...
	if (Object.values(schema).some((s) => !s.value)) {
		lines.push(
			"Чтобы <b>запустить провайдер</b>, сперва настройте все ключи 🔑",
		);
	} else {
		...
		kb.text(
			toggleText,
			`connector:${connector.active ? "stop" : "active"}`,
		).row();
		kb.text("🧪 Тест провайдера", "payments:connector").row();
	}

	Object.entries(schema).forEach(([key, { label, value }]) => {
		kb.text(
			`🔑 ${label}: ${value ?? "НЕ УКАЗАН"}`,
			`connector:key-${key}`,
		).row();
	});
