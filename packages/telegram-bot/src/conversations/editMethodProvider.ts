...
export default async function editMethodProvider(
	...
	if (data === 'pm:delete') {
		await conversation.external(() =>
			methodsApi.updateMethod(methodId, { byProvider: null })
		)
	} else if (data?.startsWith('pm:')) {
		await conversation.external(() =>
			methodsApi.updateMethod(methodId, { byProvider: data.split(':')[1] })
		)
	}
