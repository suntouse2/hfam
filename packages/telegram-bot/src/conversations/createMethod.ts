...
export default async function createMethod(
	...
	// --- Шаг 1. Название ---
	const namePrompt = await ctx.reply(
		'📝 Укажите <b>название метода</b> (или пропустите):',
		{
			reply_markup: kb,
			parse_mode: 'HTML',
		}
	)
	const nameResp = await conversation.waitFor([':text', 'callback_query'])
	...
	// --- Шаг 2. Изображение ---
	const imgPrompt = await ctx.reply(
		'🖼️ Пришлите ссылку на <b>иконку</b> (или пропустите):',
		{
			reply_markup: kb,
			parse_mode: 'HTML',
		}
	)
	const imgResp = await conversation.waitFor([':text', 'callback_query'])
	...
	await conversation.external(() =>
		methodsApi.createMethod({ projectId, label: label || '', imageSrc: imageSrc })
	)

	// --- Обновление списка ---
	const { message, kb: listKb } = await viewsMethodsList(projectId)
	await ctx.reply(message, { reply_markup: listKb, parse_mode: 'HTML' })
}
