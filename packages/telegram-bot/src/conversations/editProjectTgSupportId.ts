...
export default async function editProjectTgSupportId(
	...
	const sent = await ctx.reply(`✏️ Введите ID Поддержки:`, {
	...
	if (response.has('message:text')) {
		...
		await conversation.external(async () => {
			await projectsApi.updateProject(projectId, {
				tgSupportId: response.message.text.replace('@', ''),
			})
		})
	}

	const { message, kb } = await conversation.external(() => viewProject(projectId))
