...
export default async function editMethodMax(
	...
	const sent = await ctx.reply(
		`Выберите максимальную сумму для метода <b>${method.label}</b>:
	...
	if (data === '-') {
		...
		await conversation.external(() =>
			methodsApi.updateMethod(methodId, { maxAmount: Number(data) })
		)
	}
