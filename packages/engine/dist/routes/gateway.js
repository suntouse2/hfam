import { handlePay } from "../handlers/handlePay.js";
import { methodsService } from "../services/methodsService.js";
...
gateway.post('/', async (req, res)=>{
    ...
    const filteredMethods = methods.filter((m)=>{
        if (m.minAmount) {
            if (m.minAmount > gateway.amount) return false;
        }
        if (m.maxAmount) {
            if (m.maxAmount < gateway.amount) return false;
        }
        return true;
    });
    res.render('pay', {
